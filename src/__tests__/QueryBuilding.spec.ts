...
const queryExamples: [
  (builder: OpaqueQueryBuilder<typeof TestModel>) => OpaqueQueryBuilder<typeof TestModel>,
  NormalizedQuery
][] = [
  // Serialize
  ...
  [
    (builder) => builder.where("title", "test").or((query) => query.where("description", "other")),
    {
      _or: [
        { key: "title", comparator: "==", value: "test" },
        { key: "description", comparator: "==", value: "other" },
  ...
  [
    (builder) =>
      builder
        .where("title", "test")
        .or((query) => query.where("description", "other").or((query) => query.where("id", 12))),
    {
      _or: [
        { key: "title", comparator: "==", value: "test" },
        {
  ...
  [
    (builder) =>
      builder.where("title", "==", "john").and((query) => query.where("description", "published").orWhere("id", 0)),
    {
      _and: [
...
export class QueryBuilding {
  ...
  queryBuilding(
    modifier: Example<(builder: OpaqueQueryBuilder<typeof TestModel>) => OpaqueQueryBuilder<typeof TestModel>>,
    query: Example<NormalizedQuery>
  ) {
    // Given a query builder building the given query
    const queryBuilder = modifier(new OpaqueQueryBuilder(TestModel));
